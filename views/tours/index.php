                ...
                <table class="table table-striped mb-0 align-middle">
                    ...
                    ?>
                                    ...
                                    <img src="<?= $isExternal ? $cover : (BASE_ASSETS_UPLOADS . $cover) ?>"
                                        class="tour-cover" alt="<?= htmlspecialchars($tour['title']) ?>">
                                    <?php else: ?>
                                    <div
                                        class="tour-cover bg-light d-flex justify-content-center align-items-center text-muted">
                                        N/A</div>
                                    <?php endif; ?>
                                    <div>
                                        <div class="fw-semibold"><?= htmlspecialchars($tour['title']) ?></div>
                                        <small
                                            class="text-muted"><?= htmlspecialchars(mb_substr($tour['description'] ?? '', 0, 60)) ?>...</small>
                                    </div>
                                </div>
                            </td>
                            <td><?= htmlspecialchars($tour['category_name'] ?? 'Chưa phân loại') ?></td>
                            <td><span
                                    class="badge bg-primary"><?= htmlspecialchars($types[$tour['tour_type']] ?? $tour['tour_type']) ?></span>
                            </td>
                            <td><?= htmlspecialchars($tour['supplier_name'] ?? '---') ?></td>
                            <td><span
                                    class="badge bg-secondary"><?= htmlspecialchars($statuses[$tour['status']] ?? $tour['status']) ?></span>
                            </td>
                            ...
                            <td>
                                <a class="btn btn-sm btn-outline-secondary"
                                    href="<?= BASE_URL ?>?r=tours_itinerary&id=<?= $tour['id'] ?>">Lịch trình</a>
                                <a class="btn btn-sm btn-outline-primary"
                                    href="<?= BASE_URL ?>?r=schedules&tour_id=<?= $tour['id'] ?>">Lịch khởi hành</a>
                                <a class="btn btn-sm btn-primary"
                                    href="<?= BASE_URL ?>?r=tours_edit&id=<?= $tour['id'] ?>">Sửa</a>
                                <a class="btn btn-sm btn-danger"
                                    href="<?= BASE_URL ?>?r=tours_delete&id=<?= $tour['id'] ?>"
                                    onclick="return confirm('Xác nhận xóa tour này?')">Xóa</a>
                            </td>
                        </tr>
                        <?php endforeach; else: ?>
                        <tr>
                            <td colspan="7" class="text-center py-4 text-muted">Chưa có tour nào</td>
                        </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
