...
<head>
  ...
  <style>
    ...
    .cal-cell{min-height:100px;border:1px solid #e5e7eb;border-radius:6px;padding:6px;font-size:12px;background:#fff}
    .cal-cell .date{font-weight:600;font-size:12px;margin-bottom:4px}
    .event-badge{display:block;font-size:11px;border-radius:4px;padding:2px 4px;margin-bottom:3px;background:rgba(102,126,234,.08);border:1px solid rgba(102,126,234,.4);color:#333}
    .event-badge small{display:block;color:#666}
  </style>
</head>
<body>
  <div class="sidebar">
    ...
    <nav class="nav flex-column">
      ...
      <a class="nav-link active" href="<?= BASE_URL ?>?r=schedules_calendar"><i class="fas fa-calendar"></i> Lịch khởi hành</a>
  ...
  <main class="main">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <div class="d-flex gap-2">
        <a class="btn btn-outline-secondary" href="<?= BASE_URL ?>?r=schedules"><i class="fas fa-list"></i> Danh sách</a>
      </div>
    </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
      ...
      ?>
      <a class="btn btn-sm btn-outline-secondary" href="<?= BASE_URL ?>?r=schedules_calendar&year=<?= $prev->format('Y') ?>&month=<?= $prev->format('m') ?>"><i class="fas fa-chevron-left"></i></a>
      <h5 class="mb-0">Tháng <?= $firstDay->format('m / Y') ?></h5>
      <a class="btn btn-sm btn-outline-secondary" href="<?= BASE_URL ?>?r=schedules_calendar&year=<?= $next->format('Y') ?>&month=<?= $next->format('m') ?>"><i class="fas fa-chevron-right"></i></a>
    </div>
