  ...
  <main class="main">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <div>
        ...
        <p class="text-muted mb-0">Người liên hệ: <?= htmlspecialchars((string)$supplier['contact_person'], ENT_QUOTES, 'UTF-8') ?> | ĐT: <?= htmlspecialchars((string)$supplier['phone'], ENT_QUOTES, 'UTF-8') ?></p>
      </div>
    ...
    <div class="mb-3">
      <span class="badge bg-primary">Loại dịch vụ chính: <?= htmlspecialchars((string)$serviceLabel, ENT_QUOTES, 'UTF-8') ?></span>
    </div>
      ...
      <div class="card-body p-0">
        <table class="table mb-0 align-middle">
          ...
          <?php if (!empty($supplier['expenses'])): foreach ($supplier['expenses'] as $e): ?>
            <tr>
              <td><?= htmlspecialchars((string)($e['tour_title'] ?? ('#'.$e['tour_id'])), ENT_QUOTES, 'UTF-8') ?></td>
          ...
          <?php endforeach; else: ?>
            <tr><td colspan="5" class="text-center py-4 text-muted">Chưa có dịch vụ/chi phí nào ghi nhận với nhà cung cấp này</td></tr>
          <?php endif; ?>
          </tbody>
        </table>
      </div>
