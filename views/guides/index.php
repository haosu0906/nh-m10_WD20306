    ...
    <form class="card mb-3" method="get">
      ...
      <div class="card-body row g-3 align-items-end">
        <div class="col-md-6">
          ...
          <input type="text" class="form-control" name="q" placeholder="Tên, email hoặc SĐT" value="<?= htmlspecialchars($_GET['q'] ?? '') ?>">
        </div>
        <div class="col-md-3">
          ...
          <select class="form-select" name="type">
            ...
            <?php foreach ($types as $key => $label): ?>
              <option value="<?= $key ?>" <?= (($_GET['type'] ?? '') === $key) ? 'selected' : '' ?>><?= htmlspecialchars($label) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="col-md-3">
          ...
          <tbody>
            <?php if(!empty($guides)): foreach($guides as $guide): ?>
              <tr>
                <td>
                  <div class="d-flex align-items-center gap-3">
                    <?php if(!empty($guide['avatar'])): ?>
                      <img src="<?= BASE_ASSETS_UPLOADS . $guide['avatar'] ?>" class="avatar" alt="<?= htmlspecialchars($guide['full_name']) ?>">
                    <?php else: ?>
                      <div class="avatar bg-light d-flex justify-content-center align-items-center text-muted">
                ...
                <td><span class="badge bg-primary"><?= htmlspecialchars($types[$guide['guide_type']] ?? $guide['guide_type']) ?></span></td>
                <td><?= htmlspecialchars(mb_substr($guide['notes'] ?? '', 0, 40)) ?></td>
                <td>
                  <a class="btn btn-sm btn-primary" href="<?= BASE_URL ?>?r=guides_edit&id=<?= $guide['id'] ?>">Sửa</a>
                  <a class="btn btn-sm btn-danger" href="<?= BASE_URL ?>?r=guides_delete&id=<?= $guide['id'] ?>" onclick="return confirm('Xóa hướng dẫn viên này?')">Xóa</a>
                </td>
              </tr>
            <?php endforeach; else: ?>
