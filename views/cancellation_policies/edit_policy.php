...
<head>
    <meta charset="utf-8">
    <title>Sửa Chính sách hủy tour</title>
...
<body>
    ...
    <main class="main">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3>Sửa Chính sách hủy tour #<?= $policy['id'] ?></h3>
            <a href="<?= BASE_URL ?>?r=cancellation_policies" class="btn btn-outline-secondary">Quay lại</a>
        </div>

        <form method="post" action="<?= BASE_URL ?>?r=cancellation_policies_update&id=<?= $policy['id'] ?>">
            <div class="card">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="tour_id" required>
                                ...
                                <option value="<?= $t['id'] ?>" <?= (int)$policy['tour_id'] === (int)$t['id'] ? 'selected' : '' ?>>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" name="name" value="<?= htmlspecialchars($policy['name']) ?>" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label">Mô tả</label>
                            <textarea class="form-control" name="description" rows="3"><?= htmlspecialchars($policy['description'] ?? '') ?></textarea>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Số ngày trước khởi hành</label>
                            <input type="number" class="form-control" name="days_before" min="0" value="<?= (int)$policy['days_before'] ?>" required>
                            <small class="text-muted">Số ngày tối thiểu trước ngày đi để áp dụng</small>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">% Hoàn tiền</label>
                            <input type="number" class="form-control" name="refund_percentage" min="0" max="100" step="0.01" value="<?= number_format((float)$policy['refund_percentage'], 2, '.', '') ?>" required>
